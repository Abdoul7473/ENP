<template>
<postulant-layout>
    <Toolbar :Title="$t('welcome.list_rejected')" :breadcrumbs="breadcrumbs">
    </Toolbar>

    <CustomDataTable :headers="headers" :items="demandes_rejetees" style="margin-left: 2%;">
        ...
        <template v-slot:item.action="{item}">
            ...
            <BtnAction icon display-icon="mdi-eye" :title="$t('table.reject_motif')" @click="motif(item)" color="success" small />
    ...
    <v-dialog v-model="dialog_motif" persistent max-width="700">
        <v-card>
            <v-toolbar dense dark color="primary" class="text-h6">Motif de rejet
                <v-spacer></v-spacer>
            ...
            <v-card-text class="pt-12">
                <v-row >
                    <v-col cols="12">
                        <v-textarea class="mx-2" disabled :label="date_rejet"  :value="motif_rejet"  outlined rows="3" row-height="25" shaped></v-textarea>
                    </v-col>
                </v-row>
            </v-card-text>
        </v-card>
...
export default {
    props: ["demandes_rejetees","nbre_renvoi","nbre_rejet","nbre_auto","nbre_annule"],
    components: {
        PostulantLayout
    },
    data() {
        return {
            ...
            breadcrumbs: [{
                ...
                {
                    text: this.$t('welcome.list_rejected'),
                    disabled: true,
                    href: "/homepostulant",
                },
            ],
    ...
    methods: {
        motif(item){
            ...
            this.date_rejet = item.date_rejet
            this.motif_rejet = item.motif_rejet
        },
        updateData() {
