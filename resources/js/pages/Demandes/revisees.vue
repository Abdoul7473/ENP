<template>
    <postulant-layout>
        <Toolbar :Title="$t('welcome.list_revised')" :breadcrumbs="breadcrumbs">
        </Toolbar>
    
        <CustomDataTable :headers="headers" :items="demandes_revisees" style="margin-left: 2%;">
            ...
            <template v-slot:item.action="{item}">
                ...
                <a :href="route('demande.pdf', { id: item.id})" target="__blank">
                    <v-icon class="me-2" title="Imprimer la demande" icon="mdi-printer" color="info" small>mdi-printer</v-icon>
                </a>
                <a :href="route('autorisation.pdf', { id: item.id})" download v-if="item.statut_id == 4 && item.payer != null" style="font-size: large;">
                    <v-icon title="Télécharger l'autorisation" color="black">mdi-download</v-icon>
                </a>
                <BtnAction icon display-icon="mdi-pencil" v-if="item.statut_id == 4 && item.payer != null"  title="Revision" @click="editItem(item)" color="warning" small />
    ...
    export default {
        props: ["demandes_revisees", "nbre_renvoi", "nbre_rejet", "nbre_auto", "nbre_annule"],
        components: {
            PostulantLayout
        },
        data() {
            return {
                ...
                breadcrumbs: [{
                    ...
                    {
                        text: this.$t('welcome.list_revised'),
                        disabled: true,
                        href: "/homepostulant",
                    },
                ],
