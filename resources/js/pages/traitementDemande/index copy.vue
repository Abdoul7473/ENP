...
<admin-layout>
        ...
        <v-card-text>
            <CustomDataTable :headers="headers" :items="demandes" class="elevation-1 custom-table">
                ...
                <template v-slot:item.action="{item}">
                    <BtnAction icon display-icon="mdi-eye" v-permission:any="'verificateur|chef_service|chef_departement'" title="Détail" @click="detailItem(item)" color="warning" small />
            ...
            <v-card-text>
                ...
                <v-card-text>
                    <v-row>
                        ...
                        <v-col cols="1">
                            <v-img :src="'../../qr_code/'+qrcode+'.png'" contain max-height="90"></v-img>
                        </v-col>
                        <v-col cols="8"></v-col>
                        <v-col>
                            <v-card-title v-if="details['demande']?.nbre_mois == null" class="font-weight-bold">{{ '(VALIDE 72H)'}}</v-card-title>
                            <v-card-title v-else class="font-weight-bold">Période: {{ details['demande']?.nbre_mois }} Mois</v-card-title>
                        </v-col>
                    </v-row>
                </v-card-text>
                <v-card>
                ...
                <v-card flat v-for="(items, date) in details['routes']" :key="date">
                    ...
                    <v-card-text v-for="item in items" :key="item.id">
                        ...
                        <v-row>
                            ...
                            <v-checkbox v-if="$page.props.auth.user.type_user_id != 1 && (donnees?.demande?.statut_id == 3 || donnees?.demande?.statut_id == 4) " v-model="item.check" disabled @change="autoriser(item,item.check,donnees?.routes)" :label="item?.check ? 'Générer' : ''"></v-checkbox>
                        </v-row>
                    </v-card-text>
                </v-card>
            </v-card-text>
    ...
    <v-dialog v-model="dialogGenerer" persistent max-width="1200" class="mx-auto">
        <v-card flat class="mx-auto">
            ...
            <v-card-actions>
                <v-spacer />
                <v-btn :disabled="form.processing" v-permission="'demande.autorise'" v-if="$page.props.auth.user.type_user_id != 1" color="error" @click="dialogGenerer = false">Annuler</v-btn>
                <v-btn :disabled="form.processing" v-if="$page.props.auth.user.type_user_id == 2" color="error" @click="annuleGeneration(donnees?.demande)">Annuler</v-btn>
                <v-btn :disabled="form.processing" v-if="$page.props.auth.user.type_user_id == 2 && ready" color="yellow" @click="changeStatus(donnees?.demande,2)">Enregistrer</v-btn>
                <v-btn :disabled="form.processing" v-permission="'demande.autorise'" v-if="$page.props.auth.user.type_user_id != 1 && ready" color="primary" @click="valideGeneration(donnees?.demande)">Valider</v-btn>
            </v-card-actions>
        </v-card>
    </v-dialog>
    <!-- DialogPdf -->
