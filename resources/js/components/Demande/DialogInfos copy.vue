    ...
    <v-dialog v-model="dialogInfosDemande" max-width="1200" class="mx-auto">
        ...
        <v-card-text>
            ...
            <v-row>
                ...
                <v-col>
                    <v-label class="font-weight-bold">Date de la demande : </v-label><v-label >{{donnees?.demande?.date_demande}}</v-label>
                </v-col>
                <v-col>
            ...
            <v-row>
                ...
                <v-col>
                    <v-label class="font-weight-bold">Type du vol : </v-label><v-label >{{donnees?.demande?.type_vol?.libelle}}</v-label>
        ...
        <v-card-text>
            ...
            <v-row>
                ...
                <v-col>
                    <v-label class="font-weight-bold">Téléphone : </v-label><v-chip-group active-class="deep-purple accent-4 white--text" column 
                    >
                        <v-chip>{{donnees?.demande?.user?.postulant?.tel}}</v-chip>
                    </v-chip-group>
        ...
        <v-card-text>
            ...
            <v-card-text v-for="(items, date) in donnees?.routes" :key="date">
                <v-card-title><v-row><v-col cols="5"></v-col><v-col><v-chip large class="font-weight-bold">{{ date }}</v-chip></v-col></v-row></v-card-title>
                <!-- contenu des routes -->
        ...
        <v-card max-width="1200" class="mx-auto" v-if="($page.props.auth.user.type_user_id == 1 && donnees?.demande?.statut_id == 1) || ($page.props.auth.user.type_user_id == 2 && donnees?.demande?.statut_id <= 2) || ($page.props.auth.user.type_user_id == 3 && donnees?.demande?.statut_id <= 3) ">
            ...
            <v-card-text>
                <v-row>
                    ...
                    <v-col v-if="$page.props.auth.user.type_user_id == 1"><v-btn color="orange" @click="changeStatus(donnees?.demande,2)"><v-icon left>mdi-send</v-icon> Envoyer au supérieur</v-btn></v-col>
                    ...
                    <v-col v-permission="'demande.autorise'" v-if="$page.props.auth.user.type_user_id != 1 && verifCheck && donnees?.demande?.statut_id == 3"><v-btn color="primary" @click="modalGenerer()"><v-icon left>mdi-send-outline</v-icon>Générer l'autorisation</v-btn></v-col>
                </v-row>
            </v-card-text>
        </v-card>
        <!-- groupe de Bouton -->
