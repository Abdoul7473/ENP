...
class ComtapleController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        foreach ($autorisations_payes as $key => $value) {
            # code...
            $montant_paye=$montant_paye+$value->prix_total;
        }
        foreach ( $autorisations_impayes as $key => $value) {
            # code...
            $montant_impaye=$montant_impaye+$value->prix_total;
            ...
            $tab = collect();
                ...
                if($routes[0]->demande->permanant == 1){
                    ...
                    foreach ($liste as $index => $numero) {
                        ...
                        $auto_impayees->push([ $item_impaye, 'numero' => $numerosString, 'montant' => $mont]);
                        $numerosString = "";
                    }
                    ...
                    foreach ($liste as $key => $numero) {
                        ...
                        if ($numero->statut == 1) {
                            ...
                            $mont = Frai::where('type_autorisation_id', $type_auto->id)->where('type', $urgence)->get()[0]->montant;
                        }
